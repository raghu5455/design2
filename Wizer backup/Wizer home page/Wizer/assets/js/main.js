...
document.addEventListener("DOMContentLoaded", function () {
  ...
  navLinks.forEach((anchor, index) => {
    anchor.addEventListener("click", function (e) {
      ...
      const offsetPosition = section.offsetTop - 0;
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  window.addEventListener('resize', function () {
    if (window.innerWidth > 767) {
      if (!$('.verical-slider ul').is(':animated')) {
        startTicker();
      }
    }
  });
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  window.addEventListener('resize', function () {
    if (window.innerWidth > 767) {
      if (!$('.verical-slider1 ul').is(':animated')) {
        startTicker1();
      }
    }
  });
...
document.addEventListener("DOMContentLoaded", function () {
  function setHeights() {
    ...
    if (viewportWidth > 767) {
      ...
      const adjustedHeight = viewportHeight - 195;
      const getStartedElement = document.getElementById('getStarted');
      getStartedElement.style.minHeight = `${adjustedHeight}px`;
      getStartedElement.style.maxHeight = `${adjustedHeight}px`;
    } else {
      const getStartedElement = document.getElementById('getStarted');
      getStartedElement.style.minHeight = '';
      getStartedElement.style.maxHeight = '';
    }
  }
...
document.addEventListener('scroll', function() {
  ...
  if (window.innerWidth <= 991) {
    ...
    const secondToLastNavLink = navLinks[navLinks.length - 2];

    if (scrollPosition >= bodyHeight - 1) {
      navLinks.forEach(link => link.classList.remove('active'));
      secondToLastNavLink.classList.add('active2');
      lastNavLink.classList.add('active1');
    } else {
      lastNavLink.classList.remove('active1');
      secondToLastNavLink.classList.remove('active2');
    }
  } else {
